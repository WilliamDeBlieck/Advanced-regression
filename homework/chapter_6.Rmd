...
Those in a lower socicio economic status have a higher proportion of cancer.
...
- Analogous Relative Risk (RR): Without accounting for other factors, birdkeepers have a risk of developing lung cancer that is about 1.94 times higher than the risk for non-birdkeepers.
...
ggplot(birds, aes(x = yrsmoke, y = log(predict(logit_model)), color = cancer)) +
  geom_point() +
...
Intercept: When all other variables are held constant (assuming bird is set to "No"), the odds of developing lung cancer decrease by approximately 95.01%

yrsmoke:  When all other variables are held constant for each additional year of smoking, the odds of developing lung cancer increase by approximately 5.42%.

Bird: Holding other variables constant, individuals who keep birds have approximately 3.25 times higher odds of developing lung cancer compared to those who do not keep birds.
...
conf_intervals_yrsmoke <- confint(model4)[2, ]
conf_intervals_bird <- confint(model4)[3, ]
...
```
Yrsmoke: <br>
This interval suggests that we can be 95% confident that the true effect of yrsmoke on the log odds of developing lung cancer is between 0.0272 and 0.0937. 


<br>
bird: <br>
This interval suggests that we can be 95% confident that the true effect of bird on the log odds of developing lung cancer is between 0.7182 and 2.2771. 
...
```{R}
nba.model0 <- glm(win_pct ~ FG, family = binomial, data = nba)
nba.model1 <- glm(win_pct ~ FG + PTS, family = binomial, data = nba)
nba.model2 <- glm(win_pct ~ FG + PTS + BLK, family = binomial, data = nba)
nba.model3 <- glm(win_pct ~ FG + PTS + BLK + avg_3P_pct, family = binomial, data = nba)
