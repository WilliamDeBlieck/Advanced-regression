...
1. **Applications that do not violate assumptions for inference in LLSR**.  Identify the response and explanatory variable(s) for each problem.  Write the LLSR assumptions for inference in the context of each study.  
    ...
    <br>
    The explanatory variable is the temperature and the response variable is number of chirps per minute.
Linearity: The number of chirps is likely linearly related to the temperature
Independence: Number of chirps is likely independent of each other.
Normallity: Number of chirps per minute are normal at each temperature.
Equal Varience: There is relatively equal variance in the number of chirps for the
temperatures.
    ...
    <br>
    The explanatory variable is shoe size and the response variable is a woman's height.
Linearity: Shoe size is likely linearly related to the hieght of a woman.
Independence: Womans height are independent of each other.
Normallity: Womens' height is normally distributed for each shoe size.
Equal Varience: Womens' height has equal varience for each show size.
...
1. **Applications that do violate assumptions for inference in LLSR**.  All of the examples in this section have at least one violation of the LLSR assumptions for inference. Begin by identifying the response and explanatory variables. Then, identify which model assumption(s) are violated. 
    ...
    <br>
    Explanatory variables are socioeconomic status and parental stability response variable is birthweight.
Linearity is violated as both explanatory variable are catagorical.
    ...
    <br>
    Explanatory variable is dose response variable is relief.
Linearity is violated as explanatory variable is catagorical and response is binary. This cannot be linear.
    ...
    <br>
    Explanatory variables are distance traveled and socioeconomic measures response variable is number of trips made. Independence: No way to know that groups from same zip code are not associated. Normallity: socioeconomic status seems like it will be very heavily skewed.
    ...
    <br>
    Expanatory variable is use of patch and response variable is post natal depression. Linearity: Catagorical variable as explanatory varaible therefor not linear. Independence: Each patient vist's score is not independent of the previous.
...
1. **Kentucky Derby.**  The next set of questions is related to the Kentucky Derby case study from this chapter.   
    ...
    <br>
    Boxplots allow me to compare all four quartiles and outliers at a glance.
It is very hard to read the catagory for good in the stacked histogram while I can easily in the boxplot
We can see in the boxplot the rough amount of each condition in comparison to other conditions while we cannot in
the boxplot.
    ...
    <br>
    In the scatter plot we can see that the relationship is not linear. In the correlation coef all we see is that
is is moderatly corralated linearly. The scatterplot helps us solve the problem the coef just tells us a result.

    c. How might you incorporate a fourth variable, say number of starters, into Figure \@ref(fig:codeds)?
    <br>
    The original code for fig 1.4 is
```{R}
ggplot(derby.df, aes(x = year, y = speed, colour = fastfactor)) +
  geom_point(aes(shape = fastfactor)) +
  geom_smooth(aes(linetype = fastfactor), method = lm, se = FALSE)
...
```{R}
ggplot(derby.df, aes(x = year, y = speed, colour = fastfactor)) +
  geom_point(aes(shape = starters)) +
  geom_smooth(aes(linetype = fastfactor), method = lm, se = FALSE) +
  scale_shape_binned()
```

This makes it so that shape is now represented by the 4th variable (in this case starters)
...
Using size is also an option

```{R}
ggplot(derby.df, aes(x = year, y = speed, colour = fastfactor, size = starters)) +
  geom_point(aes(shape = fastfactor)) +
  geom_smooth(aes(linetype = fastfactor), method = lm, se = FALSE)
```
    ...
    <br>
    The vertical distance from a data point to the regression line is essentially the difference between the observed value
\begin{equation} \(Y_{i}\) \end{equation}
and the predicted value \begin{equation} 
\(\hat{Y}_{i}\) (where \(\hat{Y}_{i} = \beta_{0}+\beta_{1}\textrm{Year}_{i}\)).\end{equation}
 This difference is precisely represented by the error term \begin{equation} \(\epsilon_{i}\). \end{equation}
    ...
    <br>
    The t-test is a statistical test used to compare the means of two groups and determine if there is a significant difference between them.
To answer this the t-test is used to assess whether the estimated slope is significantly different from zero.
We find this by finding the estimated slope relative to the standard error
    ...
    <br>
    The independent-samples t-test is used to compare the means of two independent groups (in this case the two catagories, Fast and not Fast).
The \beta_{1} t-test is used to test whether there is a significant difference in the means of the dependent variable 
Y between the two levels of \textrm{Fast}_{i}
Equal varience is a needed assumtion for this due to the fact that it is needed for an accurate t-test in the first place.
    ...
    <br>
    It is important due to the fact that all other variables must be held constant in multi linear regression to disscuss any individual predicting variables affect.
A fixed year qualifies this as such.
...
I can say with 95% confidence that 1896 had a winning speed between a mean of 50.61 and 51.22 feet per second
    i. State (in context) the result of a t-test for $\beta_1$ in Model 4.
```{r}
model4_sum <- summary(model4)
model4_sum$coef
```
The null hypothesis is rejected as the p-value is 1.117e-21 which is below .05 significance. The count of years since 1896 is significant.

    j. Why is there no $\epsilon_i$ term in Equation \@ref(eq:model5est)?
    <br>
    equation 1.4 is an estimated model therefor it does not not include the error.
    ...
    <br>
    For every unit increase in yearnew the responce will increase by starters multiplied by the interaction coef.(ask)
    l. Interpret (in context) the LLSR estimates for $\beta_3$ and $\beta_5$ in Equation \@ref(eq:model0).  
    <br>
    Holding all other variables constant, when the track condition is fast the winning speed will increase by 1.39 ft/s.<br> Holding all other variables constant, for each added horse the winning speed will decrease by .03 ft/s
...
1. **Moneyball.**  In a 2011 article in *The Sport Journal*, Farrar and Bruggink [-@Farrar2011] attempt to show that Major League Baseball general managers did not immediately embrace the findings of Michael Lewis's 2003 *Moneyball* book [@Lewis2003].  They contend that players' on-base percentage remained relatively undercompensated compared to slugging percentage three years after the book came out.  Two regression models are described: Team Run Production Model and Player Salary Model.   
    ...
    <br>
    Team Run Production: <br>
    Linearity: There is likely linearity in the continous responce and explanatory varaibles <br>
    Independence: Baseball teams react to one another. I belive there are dependencies between teams. <br>
    Normalaity: There should be a normal distribution <br>
    Errors: There should be equal varaience. <br>
    Player Salary Model: <br>
    Linearity: There is a quadratic variable (\beta_{3}G^{2}) so linearity is questionable.<br>
    Independence: Players on their team are not independent of each other. <br>
    Normality: There may be a non normal distribution of pay for players.<br>
    Equal varience: Once again there may be a changing varience for player pay.
    ...
    <br>
    It seems strange to argue that model 1 is better when model 3 has a higher adj R sqrd  value. We could formally test this with a nested f test.
...
NL:OBP, represents a disparity (0.275) in the rate of run increase associated with OBP between the team categories. NL:SLG represents a difference (0.241) in the slope of run increase based on SLG for team categories.
    ...
    <br>
    They most likley mean that their model explains more runs scored than the Hakes and Sauer model. This is most likely not relevent as the focus on and controll for different variables.
    ...
    <br>
    The extra variables may not be significant, but it does help to explain more of the data. There is more data explained by this extra variable than is penalized by its addition.
    
    f. What limits does this paper have on providing guidance to baseball decision makers?
<br>
This paper is a statistical analysis on the adoption of ideas from the book moneyball.
Many concepts are not all that relevent for actual predictions unless the person wants to 
look at how OTB is currently valued in the league.
...
```{R}
bank_salary <- read.csv("data/banksalary.csv")
bank_salary <- bank_salary %>%
  mutate(sex_indicator = case_when(
    sex == "MALE" ~ 1,
    TRUE ~ 0
  ))
```
    ...
    <br>
    The observational units are skilled, entry-level clerical workers who worked for the bank between 1965 and 1975. The explanatory variables are sex, age, educ (education), and exper (experience). The response variable is starting salary.
...
```{R}
bank_salary %>% group_by(sex_indicator) %>%
summarise(mean(bsal))
```
This is not enough proof. Female employees may have less education or different age compared to their male counterparts. The other explanatory variables must be explored. 
    ...
    <br>
    I would expect experience, and education to be strongly positivly cooralated with starting salary. I would also expect age to have a weaker positive cooralation with starting salary as age is most likely cooralated with experience.
```{R}
ggplot(bank_salary, aes(x = bsal, y = age)) +
  geom_point() +
  labs(x = "Salary", y = "Age") +
  ggtitle("Scatter Plot of Salary vs Age")

ggplot(bank_salary, aes(x = bsal, y = educ)) +
  geom_point() +
  labs(x = "Salary", y = "Education") +
  ggtitle("Scatter Plot of Salary vs Education")


ggplot(bank_salary, aes(x = bsal, y = exper)) +
  geom_point() +
  labs(x = "Salary", y = "Experience") +
  ggtitle("Scatter Plot of Salary vs Experience")

cor(bank_salary$bsal, bank_salary$age)
cor(bank_salary$bsal, bank_salary$educ)
cor(bank_salary$bsal, bank_salary$exper)

```
Experience is not as correlated as I thought it would be. I am confused as to why this is. Age's correlation is very weak as I predicted. Education is correlated as I expected. This means that I propably should not use Age in my model.
    ...
    <br> Less competitive (thereby less paid) candidates might come and go much quicker than the more skilled ones.
...
```{r}
cor(bank_salary %>%
  select(sex_indicator, exper, educ, age, bsal))

ggplot(bank_salary, aes(x = age, y = exper)) +
  geom_point() +
  labs(x = "Age", y = "Experience") +
  ggtitle("Scatter Plot of Age vs Experience")
```

Age and experience seem to be postively linearly correlated as previous predicted. This means that we should not use one of these predictor and as age is the worse predictor we should use experience.
...
```{r}
bank.model1 <- lm(bsal~exper, data=bank_salary)
summary(bank.model1)

```

When the candidate has no experience the starting salary is 5289. For every month of experience the starting salary increases by 1.30 (Wow that is little). This model explains 2.8\% of the variability in the data. Experience is not a significant predictor.
...
plot(bank.model1)
```
Linearity: Residuals vs Fitted shows a pattern that leads me to believe the there isn't linearity
<br>
Independence: If employees are randomly sampled we can assume independence.
Normality: Normal Q-Q plot show normality as most residuals are close to the line.
Equal variance: Points have less variation as x increase wich leads me to believe that there isn't equal variance.
...
```{R}
bank.model2 <- lm(bsal~senior+educ+exper+age, data=bank_salary)
summary(bank.model2)
```

```{R}
cat(sprintf("R-Squared Model 1 = %.4f\nR-Squared Model 2 = %.4f",
            summary(bank.model1)$r.squared, summary(bank.model2)$r.squared))
```
Model 2 has a much higher R^2 value (.318 compared to .028). That means that model 2 can explain much more of the variability than model 1.
<br>
The best way to compare two models though as previously talked about under the Moneyball questions is to run a nested f tested
```{R}
anova(bank.model1, bank.model2, test = "F")
```
Model2 is significantly better at predicting starting salary than model1.
    ...
    <br>
    We should stop using age as it correlated far too much with experience and is not significant.
...
```{R}
ggplot(data=bank_salary, aes(x=age, y=exper)) +
  geom_point() +
  labs(x = "Age", y = "Experience") +
  ggtitle("Scatter Plot of Age vs Experience")
```
There is a positive association where as experience increases both age and the variability of age increases.
...
```{R}
bank.model3 <- lm(bsal~sex_indicator+senior+educ+exper, data=bank_salary)
summary(bank.model3)
...
plot(bank.model3)
```
Linearity: The assumption of linearity passes
Independence: The assumption of independence passes
Normality: The assumption of normality passes
Equal Variance: The assumption of equal variance is questionable due to the cluster and not straight line in res vs lev graph. More data may be needed.

```{R}
confint(bank.model3)
```
Holding all other variables constant, I can say with 95% confidence that the mean increase in beginning salary for being male compared to being female is between \$488.15 and \$956.45.
    ...
    Holding all other variables looked at in this model constant there were clear signs of gender discrimnation. Some things were not looked at, like for example how good the previous experience is. This needs to be qualified by that we only looked at age, experience, sex, and seniority and that this is holding all other variables constant.
...
shapiro.test(bank_salary$bsal)

logged_salaries <- log(bank_salary$bsal)

shapiro.test(logged_salaries)
```
It looks like a log transformation should be done on the salary as the p value significantly increased in the shapiro wilk test after the transformation was done.
...
```{R}
bank.model3 <- lm(log(bsal)~senior+educ+exper+sex_indicator, data=bank_salary)
exp(coef(bank.model3)["sex_indicator"])
```

The interpretation is that holding all other variables constant, on average, males have an estimated multiplicative effect of 1.138 on starting salaries compared to females.
...
<br>
```{R}
bank.finalmodel <- lm(bsal~sex_indicator+exper+educ+senior+sex_indicator:senior, data=bank_salary)
summary(bank.finalmodel)

```
```{R}
bank.finalmodel <- lm(bsal~sex_indicator+exper+educ+senior+sex_indicator:exper, data=bank_salary)
summary(bank.finalmodel)

```
```{R}
bank.finalmodel <- lm(bsal~sex_indicator+exper+educ+senior+sex_indicator:educ, data=bank_salary)
summary(bank.finalmodel)

```

```{R}
bank.finalmodel <- lm(bsal~sex_indicator+exper+educ+senior+sex_indicator:educ+sex_indicator:senior, data=bank_salary)
summary(bank.finalmodel)

```
```{R}
bank.finalmodel <- lm(bsal~sex_indicator+exper+educ+senior+sex_indicator:educ+sex_indicator:exper+sex_indicator:senior, data=bank_salary)
summary(bank.finalmodel)

```
```{R}
bank.finalmodel <- lm(bsal~sex_indicator+exper+educ+senior+sex_indicator:educ+sex_indicator:exper, data=bank_salary)
summary(bank.finalmodel)

```
```{R}
bank.finalmodel_no_sex <- lm(bsal~exper+educ+senior+sex_indicator:educ+sex_indicator:exper, data=bank_salary)
summary(bank.finalmodel_no_sex)

```

```{R}
confint(bank.finalmodel)
``` 
There is significant interaction between a workers sex and education and sex and experience. I then removed sex_indicator from my final model and got a higher adj r sqrd value. This shows that there is most likely not gender discrimination going on and instead an interaction with a workers education and sex and experience and sex and both of those affect starting salary.
...
summary_stats(houses$price)
summary_stats(houses$sqft)

    
    
ggplot(houses, aes(y = price)) +
  geom_boxplot() +
  labs(y = "Price") +
  ggtitle("Boxplot of House Prices")

ggplot(houses, aes(y = sqft)) +
  geom_boxplot() +
  labs(y = "sqft") +
  ggtitle("Boxplot of House Square Footage")

ggplot(houses, aes(x = price, y = sqft)) +
  geom_point() +
  labs(x = "Price", y = "Square Footage") +
  ggtitle("Scatter Plot of Price vs Square Footage")
    
    ```
    There seems to a postive linear relationship between price and sqft where as price increases sqft increases in value and increases in variability.
...
```{R}
houses.model1 <- lm(price~sqft, data=houses)
plot(houses.model1)

summary(houses.model1)
```

Every assumption excpet for indepences does not pass. Independence only passes if houses are randomly sampled.
...
houses.model2 <- lm(logprice~sqft, data=houses)
coef(summary(houses.model2))
```

logprice = 12.221846 + 0.0003987sqft
    d. How does `logprice` change when `sqft` increases by 1?
    
    logprice increases by .0003987 for every 1 increase in sqft
    e. Recall that $\log(a) - \log(b) = \log\big(\frac{a}{b}\big)$, and use this to derive how `price` changes as `sqft` increases by 1.
```{R}
    exp(coef(summary(houses.model2)))
```
The price increases by 1.000399 when the sqft increases by 1.
...
Linearity: The residuals vs fitted plot shows that we do not pass the assumtion of linearity
<br>
Independence: As long as houes are randomly sampled we pass the assumption of independence.
<br>
Normallity: The Normal Q-Q graph looks roughly normal
Equal Varience: No influential points in res vs levrage
...
houses.model3 <- lm(price~logsqft, data=houses)
coef(summary(houses.model3))
```
 price = −3451377.1 + 528647.5logsqft
    
    h. How does predicted `price` change as `logsqft` increases by 1 in Model 3?
As logsqft increases by 1 price increases by 528647.5.
...
As sqft increases by 1% price increases by 5260.218
...
Linearity: Massive curve in res vs fitted. Does not pass
Indepdence: As long as houses are a random sample pass
Normallity: Massive curve in normal Q-Q. Does not pass
Equal Varience: No points of intrest in res vs levrage
...
```{R}
houses.model4 <- lm(logprice~logsqft, data=houses)
summary(houses.model4)
```
 logprice = 6.7299 + 0.8368logsqft
 
 
    l. In Model 4, what is the effect on `price` corresponding to a 1\% increase in `sqft`?
    
```{R}
coef(summary(houses.model4))
```

```{R}
exp(log(1.01) * 0.836771)
```
...
```
Linearity: Fitted vs res plot shows curve. Does not pass
Independence: As long as houses are randomly sampled pass
Normality: Normal Q-Q Normallity is showen all points close to line pass
Equal Varience: No points of intrest.
...
```{R}   
houses.finalmodel <- lm(logprice~logsqft+bedrooms, data=houses)
summary(houses.finalmodel)
```
    Adding in Bedrooms increase adj r^2 from .4555 to .4647. For every extra bedroom the logprice  decreases by .0695
...
```{R}
films <- read.csv("data/bechdel.csv")
films <- na.omit(films)

```

```{R}
#Not looking at intgross domgross introi domroi as they prob correlate with totals
cor(subset(films, select=c(pass, year, budget, totalGross, totalROI)))

```

```{R}
# Select relevant columns from your dataset
films <- films[c("totalROI", "year", "budget", "totalGross", "pass")]

# Loop through pairs of variables
for (i in 1:(ncol(films)-1)) {
  for (j in (i+1):ncol(films)) {
    
    # Calculate correlation between variables i and j
    correlation_test_ij <- cor.test(films[[i]], films[[j]])
    correlation_coefficient_ij <- correlation_test_ij$estimate
    r_squared_ij <- correlation_coefficient_ij^2
    
    cat("\nCorrelation between", names(films)[i], "and", names(films)[j], "is:", correlation_coefficient_ij)
    cat("\nR-squared:", r_squared_ij)
    
    # Check if i is not "totalROI"
    if (names(films)[i] != "totalROI") {
      # Calculate correlation and r^2 with the interaction term of i and j
      interaction_term <- films[[i]] * films[[j]]
      correlation_test_interaction <- cor.test(interaction_term, films$totalROI)
      correlation_coefficient_interaction <- correlation_test_interaction$estimate
      r_squared_interaction <- correlation_coefficient_interaction^2
      
      cat("\nInteraction Correlation between", names(films)[i], ":", names(films)[j], "and totalROI is:", correlation_coefficient_interaction)
      cat("\nInteraction R-squared:", r_squared_interaction)
    }
  }
}
...
```
Budget and totalgross have and r^2 of .48. They seem to be compounding. Budget has a higher r^2 with pass so I shall only use it.

```{R}
films.model1 <- lm(totalROI~pass, data=films)
summary(films.model1)
```

As seen previously the adj r sqrd is very small for this model. Pass is not a good predictor for TotalROI

```{R}
films.model2 <- lm(totalROI~totalGross + totalGross:pass + pass + budget, data=films)
summary(films.model2)
```
```{R}
films.model3 <- lm(totalROI~totalGross + totalGross:pass + budget, data=films)
summary(films.model3)
```

```{R}
films.model4 <- lm(totalROI~totalGross + pass + budget, data=films)
summary(films.model4)
```

```{R}
films.model5 <- lm(totalROI~totalGross + budget, data=films)
summary(films.model5)
```
As seen in the models 2-5 adj r^2 is highest for totalGross + totalGross:pass + pass + budget.

Still this model is not helpful in finding ROI. New parameters would be needed to accuractly model ROI.

```{R}
plot(films.model2)
```
Line is not passed at all for this.
Linearity: linearity is possibile but slight curve is seen.
Indpendence: As long as movies are randomly sampled independence is pass
Normallity: Normal Q-Q shows no normallity not pass
Equal variance: Not pass

Only less than 5% of the data is ever explained by the model and the model does not pass LINE. The Bechdel test is not a good predictor for ROI in films. Other parameters would be needed to properly model ROI.
