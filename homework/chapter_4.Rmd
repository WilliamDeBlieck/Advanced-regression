...
total students, all sat and act predictors, and tuition.
...
```
We have some normallity and linearity issues as seen in normal qq and res vs fit
...
For a one-unit increase in tuition, the expected log-count of burglaries decreases by approximately 0.03896, holding other predictors constant.
...
summary(fit3)
```
For a one-unit increase in tuition in thousands, while holding other variables constant, the log-count of burglaries is expected to increase by 0.145144. 

<br>

For a one-unit increase in act.comp and a one-unit increase in tuition in thousands, the log-count of burglaries is expected to decrease by 0.007247. 

<br>

For a one-unit increase in act.comp and a one-unit increase in total in thousands, the log-count of burglaries is expected to increase by 0.009273.
...
```
Holding other variables constant, each one-unit increase in the chronic variable is associated with a 11.87% increase in the expected count of physician office visits.

Individuals with poor health are expected to have 32.47% more physician office visits, holding other variables constant.
  
Individuals with excellent health are expected to have 26.65% fewer physician office visits, holding other variables constant.

Holding other variables constant, individuals with insurance are expected to have approximately 15.14% more physician office visits.

The log-odds of excess zeros (individuals not visiting a physician office) is reduced by approximately 0.37 when all other predictor variables are zero.

Holding other variables constant, each one-unit increase in the chronic variable is associated with a decrease in the log-odds of excess zeros by approximately 0.56.

Holding other variables constant, individuals with insurance are associated with a decrease in the log-odds of excess zeros by approximately 0.88 compared to those without insurance.
...
poisson_model <- glm(number_of_reviews ~ room_type + bathrooms + bedrooms + price + 
                        ...
                        review_scores_value + instant_bookable + offset(log(days)),
                      family = "poisson", data = airbnb)
...
The p-values associated with each coefficient suggest the significance of each predictor variable. All predictors have extremely low p-values, indicating their statistical significance.
